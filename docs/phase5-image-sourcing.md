...
- 完成：
  ...
  - 从 Swarovski 中国官网商品列表提取带商品链接、variantID 与 alt 文本的 SKU 级图片，替换 7 条结构示意图。
  - 新增 `scripts/phase5-official-image-fix.mjs`，可复跑本轮图片状态修正。
  - 新增 `scripts/phase5-sku-image-swarovski.mjs`，记录 Swarovski 逐 SKU 图片映射。
- 未完成：
...
| official-remote 真官方远程图 | 40* | 13 |
| generated-local 结构示意 | 141 | 130 |
| placeholder 结构示意 | 12 | 50 |
| 结构示意合计 | 153 | 180 |
...
| Swarovski | 7 | `https://asset.swarovski.com/images/...` Swarovski 中国官网商品 SKU 图 |
...
Image status:
  ...
  official-remote: 13
  generated-local (illustration): 130
  placeholder: 50
  missing: 0
...
1. 优先逐 SKU 深挖 Gucci、Chanel、Pandora、Cartier、Tiffany 的商品页，而不是使用分类页图。
2. 对 `official-remote` 增加可选字段 `imageVerifiedAt` 和 `imageMatchConfidence`。
3. 未来若找不到真实官方图，继续保留“结构示意”比误标为官方图更安全。

## 6. 本轮未替换原因

- Chanel：Coco Crush 页面当前稳定抓到的是品牌宣传图；未能稳定获取具体 Coco Crush 戒指 SKU 图，因此暂不替换。
- Gucci Link to Love：官网可抓到专题入口图，但不是具体戒指 SKU 图，因此暂不替换。
- Pandora：官网页面在本轮抓取中连接关闭，暂不替换。
